...
@About
$ # FAR: что это такое#
    #FAR2M# - это работающая в текстовом режиме программа управления файлами, которая
имеет обширный набор дополнительных функций.

    #FAR2M# является #freeware# и #open source# программой, распространяемой
под GNU GPL v2 ~лицензией~@License@.

    #FAR2M# позволяет работать с #архивами#. При этом файлы в архивах
обрабатываются аналогично файлам в папках. #FAR2M# сам преобразует ваши команды в
...
@CmdLine
$ # FAR2M: параметры командной строки#
  Вы можете задавать следующие аргументы в командной строке или переменной окружения FAR2M_ARGS:

  #--tty#
  Запускает far2m сразу в ~режиме TTY~@UIBackends@ вместо автоматического определения возможности
...
опыта, иногда может потребоваться запуск строго в терминальном режиме.
  ...
  #--nodetect#
  По умолчанию far2m пытается на запуске определить, не работает ли он в терминале другого
far2m. В этом случае far2m автоматически использует режим TTY с расширениями терминала far2m. В
случае отсутствия таких расширений терминала - far2m проверяет наличия доступа к X11 сессии и
...
расширений и разрешает к использованию лишь базовые возможности обычного терминала в TTY режиме.
  ...
  #--mortal#
  Этот аргумент применим только для far2m, который работает в режиме TTY. По умолчанию при закрытии
терминала far2m продолжает работать 'в фоне' и при последующем запуске другого far2m в режиме TTY
будет предложено активировать ушедший в фон far2m вместо запуска новой копии приложения. Этот ключ
командной строки полностью отключает эту функциональность, так что закрытие терминала будет приводить
к безусловному завершению работы far2m, работающего в нем.

  #--primary-selection#
  Использовать PRIMARY selection вместо CLIPBOARD X11 selection. Этот аргумент применим только для
far2m, который работает в режиме WX.

  #-a#
  Запрет отображения символов с кодами 0 - 31 и 255. Может быть полезно при
запуске FAR2M из telnet.
...
пути поиска можно использовать переменные среды.
  ...
  #-co#
  Заставляет FAR2M загружать внешние модули только из кэша. Загрузка при этом происходит
...
надо загрузить FAR2M без этого параметра. Если кэша нет, то модули загружаться не будут.

  Замечания относительно ключей -p и -co:

  - ^<wrap>если указан пустой -p, то FAR2M запустится без плагинов;
...
"~Путь к персональным плагинам~@PluginsManagerSettings@"
  ...
  #-m#
  При старте FAR2M не будет загружать макросы.

  #-ma#
  При старте FAR2M не будет выполнять макросы с опцией "Выполнять после запуска FAR2M".

  #-u <username>#
  Позволяет использовать раздельные настройки для различных пользователей.
  Например: far -u guest

  FAR2M установит ~переменную среды~@FAREnv@ "FARSETTINGS" в значение, равное <username>.
...
в указанном примере), то FAR2M будет ждать завершения входного потока вечно.
Это особенность текущей версии FAR2M.
...
используйте #-cd <path>#) или двух команд с префиксом плагина. Первый путь для активной панели,
второй - для пассивной:
  - ^<wrap>для каталогов и архивов FAR2M покажет их содержимое;
...
команда для пассивной панели (пассивная панель при этом временно активируется). Односимвольные префиксы игнорируются.
  #Пример:# far2m ma:~~/Downloads/far2m.7z "lua: msgbox(\\"FAR2M\\",\\"Successfully started\\")"
...
FAR2M также позволяет использовать числовую сортировку.
...
ошибочной раскладке клавиатуры. В этом случае FAR2M перекодирует набранный текст
...
осуществляется через него, иначе - средствами FAR2M.
...
файлов, ссылающихся на одни и те же данные.
  ...
  Изменение размеров окна FAR2M                         #Alt-F9#
...
сессии FAR2M, используется кодовая страница по умолчанию.
...
командой, исполняемой программой или пакетным файлом.
    ...
    При запуске содержимого командной строки FAR2M ищет исполняемый
...
непустой ~командной строки~@CmdLineCmd@ будет исполнено её содержимое.
   ...
   FAR2M также умеет обрабатывать ~колесо мыши~@MsWheel@.
...
создания новых команд FAR2M и поддержки дополнительных файловых систем. Например,
...
'Plugins', которая находится в одной папке с FAR2M.EXE. При обнаружении нового
модуля FAR2M сохраняет информацию о нём и впоследствии загружает его только при
...
список доступной помощи по модулям, нажав:
    ...
    #Shift-F2# - в основной помощи FAR2M
...
помощи, то помощь вызываться не будет.
    ...
    При вызове контекстной помощи FAR2M попытается показать тему с именем
...
$ #Обзор возможностей подключаемых модулей#
    Оболочка FAR2M настолько тесно слилась со своими плагинами, что
говорить о ней и не говорить о плагинах просто бессмысленно. Плагины неимоверно
расширяют возможности FAR2M, дополняя их и модифицируя.

@Panels
$ #Панели#
    Обычно FAR2M показывает две панели (левое и правое окна) с различной
...
над папками.
    ...
    FAR2M запоминает информацию о структуре папок в файле #tree2.far#,
...
переименование, копирование) FAR2M отслеживает и обновляет его
внутреннюю структуру. Если изменения были сделаны не в FAR2M обновить
...
команды "Файлы описания папок" в ~Меню параметров~@OptMenu@.
    ...
    Для приводов компакт-дисков FAR2M умеет определять тип привода. Различаются
...
@Terminal
$ #Терминал#
    #FAR2M# содержит встроенный эмулятор терминала, позволяющий исполнять приложения командной строки и управлять ими, наблюдая их вывод.
Для соблюдения привычного пользовательского опыта far2m запускает приложения внутри поддерживаемой интерактивной оболочки командной строки.
    #Автодополнение# FAR2M имеет два независимых механизма автоматического дополнения командной строки. Первый является механизмом автодополнения,
...
командной оболочки bash и может быть активирован нажатием SHIFT+двойной-TAB (быстро дважды нажать TAB при нажатом SHIFT).
    #Поведение команды exit:# ввод команды 'exit' приведет к закрытию оболочки командной строки, но не закроет само приложение far2m, а последующий
...
Если же вы хотите закрыть far2m с командной строки: напечатайте псевдокоманду 'exit far' - она будет распознана far2m как команда закрытия.
...
вызвать повреждение или не сохранение данных убитого приложения. Если far2m работает в TTY то вы так же можете использовать сочетание Ctrl+Alt+Z чтобы поместить far2m в фон, освободив терминал, но продолжая исполнять текущую команду.
...
панелями возможно нажатием сочетания Ctrl+O или щелчком по левому верхнему углу окна.
    #Расширения терминала FAR2M# хотя FAR2M сам по себе это приложение тeкстового режима, оно может работать в режиме вывода через подсистему
...
но он так же может работать внутри терминала другого far2m, получая от этого дополнительные возможности, недоступные под 'обычными' терминалами,
такие как распознавание всех кнопок клавиатуры и событий нажатия/отпускания кнопок. Так же 'хостовый' far2m может предоставлять доступ к буферу
обмена хоста и механизм уведомлений рабочего стола. Вы можете использовать эту функциональность запуская TTY far2m на SSH сервере внутри SSH сессии
клиента, запущенного внутри терминала far2m, работающего в GUI режиме на хосте клиента. Или, что намного проще, используя плагин с возможностью
...
@UIBackends
$ #Режимы интерфейса#
    В зависимости от настроек билда и доступных возможностей платформы #FAR2M# может выводить
...
и для получения состояний всех клавиш клавиатуры. Это обеспечивает самое удобное использование в обычном терминале.
    - #Режим TTY|F:# выводит интерфейс в терминал созданный другим приложением far2m instance. Это обеспечивает уровень
удобства использования аналогичный режиму GUI (если терминал предоставлен GUI версией far2m).

    Если вы хотите запустить far2m удаленно с максимальным удобством, то рекомендуется либо запустить его из сессии
подключения NetRocks, что позволяет использовать TTY|F режим. Если же это невозможно - рекомендуется запускать far2m
...
единственным безопасным методом удаленного запуска far2m на недоверенном сервере с поддержкой всех привычных
...
$ #Менеджер внешних модулей#
  ...
  Укажите полный путь, где FAR2M будет искать "персональные" плагины в дополнение к "основным". Можно указать
несколько путей поиска, разделённых символами ':' или ';'. При указании пути поиска можно использовать переменные среды.
...
символом "#/#". Например: #$USERDOMAIN/$USER/Folder3#.
...
разделены символом-разделителем "#;#" или "#,#".
 #-# ^<wrap>Если эта опция включена и имя папки содержит в себе разделители
и/или пробелы, то оно должно быть взято в кавычки.
 #-# ^<wrap>Например, если будет введено #/Foo1;"/foo,2;";/foo3#, то
будут созданы папки с именами: #/Foo1#, #/foo,2;# и #/foo3#.
 #-# ^<wrap>Если имя папки заключено в кавычки, то каждая кавычка внутри имени
должна быть удвоена.
...
местное меню в папке FAR2M, то оно будет использовано вместо хранящегося в
...
@FileAssoc
$ #Ассоциации файлов#
    FAR2M поддерживает ассоциации файлов, которые позволяют задавать
...
то FAR2M пытается использовать ассоциации Windows для запуска этого типа
файлов.


@FileAssocModify
$ #Настройка ассоциации файлов#
    FAR2M позволяет задать шесть команд, ассоциированных с определённым типом
...
в ~Системных параметрах~@SystemSettings@, то FAR2M пытается использовать
...
метасимволы:
    ...
    #!@@!# or #!$!#    Имя файла, содержащего имена помеченных файлов
  ...
  Примечания:
    ...
    1. ^<wrap>FAR2M при обработке метасимволов подставляется только то, что они означают
(имя файла, расширение и т.д.). Никаких дополнительных символов (например,
кавычек или фамилий разработчиков FAR2M) не подставляется и вы должны это сделать
...
каталогов и определении суммарного размера файлов в подкаталогах.
  ...
  #Время бездействия#
  Завершает работу FAR2M, если в течение указанного интервала не было нажатий клавиш мыши или клавиатуры,
FAR2M ожидал ввода из командной строки и отсутствовали фоновые экраны редактирования или просмотра.

  #Сохранять историю команд#
  Вызывает сохранение ~истории команд~@History@ перед завершением и её восстановление после запуска FAR2M.

  #Сохранять историю папок#
  Вызывает сохранение ~истории папок~@HistoryFolders@ перед завершением и её восстановление после запуска FAR2M.
...
после запуска FAR2M. Для просмотра входящих в этот список файлов нажмите #Alt-F11#.
...
в ~ассоциациях файлов~@FileAssoc@ FAR2M, будет запущена программа Windows, предназначенная для обработки этого типа файлов.

  #Автоматически монтировать CDROM#
  При выборе компакт-диска в меню ~выбора дисков~@DriveDlg@ FAR2M задвинет открытый лоток привода компакт-дисков. Отключите эту опцию,
...
некоторых приводов)
  ...
  При выходе из FAR, конфигурация будет автоматически сохраняться,
за исключением состояния панелей.

  #Автосохранение состояния панелей#
  Состояние панелей автоматически сохраняется при выходе из FAR.
...
$ #Настройка панели#
  ...
  #папок по расширению#     не только к файлам, но и к папкам. При
                          ...
                          работает так же, как и в FAR2M 1.65.
...
комбинации кнопкосочетания будет заставлять FAR2M эксклюзивно захватывать
...
мыши в правый верхний угол окна FAR2M (при выбранной опции).
...
включая следующие переменные:
  - #%State# - текущее состояние FAR2M (ранее было заголовком окна по умолчанию);
  - #%Ver# - версия FAR2M;
  - #%Platform# - архитектура FAR2M;
  - #%Backend# - режим интерфейса FAR2M;
  - #%Host# - имя машины;
  - #%User# - имя пользователя;
  - #%Admin# - слово "Root", если FAR2M запущен от имени администратора, иначе пустая строка.


@DialogSettings
$ #Настройка диалогов#
  #История в строках#       Сохранять историю в строках ввода
  #ввода диалогов#          некоторых диалогов FAR2M. Список ранее
...
показом панелей. Допустимые значения: Никогда/При ошибке/Всегда.
  ...
  Если оболочка не задана, far2m попытается использовать системную (#$SHELL#). Если системная оболочка
не соответствует внутренним критериям far2m, то будет использоваться #bash#.
...
ошибками.
  ...
  Отображать текстовый блок с версией far2m, копирайтами и подсказками по горячим клавишам под панелями при запуске.
...
$ #Формат командной строки#
   FAR2M позволяет изменить формат приглашения командной строки.
...
специальные коды:
     ...
     $l - символ <
     $## - символ ## если FAR2M запущен под root, иначе $
...
$ #Программа просмотра: клавиши управления#
   Команды программы просмотра
    ...
    #Alt-F5#             Печать файла (используется плагин "Print Manager").
    ...
    #Alt-F9#             Изменение размеров окна FAR2M
    ...
    #Ctrl-U#             Сбросить выделение, полученное в результате поиска.
    ...
    3. В текущей версии FAR2M существует ограничение на максимальное
    ...
    4. FAR2M ~ищет~@ViewerSearch@ первое вхождение подстроки (#F7#) с начала видимой
...
и/или образец, который будет использован для исключения строк из вывода.
    Возможно также указать #количество строк до/после совпавшей строки#, которые также будут включены в вывод.
...
появится предупреждение: "~Путь к редактируемому файлу не существует~@WarnEditorPath@".
    ...
    2. ^<wrap>При попытке повторного открытия на редактирование файла, FAR2M выдаст
...
~Настройки редактора~@EditorSettings@.
  ...
  Операции над блоками
   ...
   #Тянуть мышь, удерживая#
     #левую кнопку#              Пометка блока
   #Ctrl-Shift-Клавиши курсора#  Пометка блока по словам
   #Alt-Клавиши курсора#         Пометка вертикального блока
   #Alt-Shift-Клавиши курсора#   Пометка вертикального блока
   #Alt + тянуть мышь,#
     #удерживая левую кнопку#    Пометка вертикального блока
...
$ #Редактор: повторное открытие файла#
    FAR2M отслеживает все попытки повторного открытия в редакторе уже
...
несуществующей папки. Перед сохранением файла FAR2M создаст такую папку при
...
совпадают с теми, которые FAR2M запомнил при последнем обращении к этому файлу.
...
$ #Определение кодировок ANSI и OEM#
  ...
  #~~/.config/far2m/cp# (первая строка - #OEM#, вторая - #ANSI#)
...
запрашивает привилегии суперпользователя.
    ...
    Настройки меню перехода сохраняются в конфигурации FAR2M.
...
только для дисков).
    ...
    Если включён показ параметров компакт-дисков (#Ctrl-8#), то FAR2M будет
...
FAR2M предусмотрена возможность цветовыделения файловых объектов.
...
$ #Автоопределение кодовой страницы#
    FAR2M постарается выбрать правильную кодовую страницу для просмотра/редактирования
...
описания, или разрешено всегда. По умолчанию FAR2M устанавливает атрибут "Hidden"
...
описаний.
    ...
    Если файл с описаниями имеет атрибут "Только для чтения", то FAR2M не
...
#описаний с атрибутом "Только для чтения"# FAR2M попытается корректно обновить
описания.

    Если это разрешено в конфигурации, FAR2M обновляет описания файлов при
...
FAR2M всегда использует настоящий регистр.
...
(запятую или точку с запятой), чтобы такая маска не была спутана со списком.
 ...
 Если маска заключена в двойные кавычки, то каждая двойная кавычка внутри маски
должна быть удвоена.
...
способов копирования встреченных символических ссылок:
    ...
    - Либо far2m будет проверять путь, указываемый каждой ссылкой чтобы понять -
...
системную функцию копирования" из диалога ~Системные параметры~@SystemSettings@.
    ...
    Параметр "Уже существующие файлы" определяет поведение FAR2M для случая,
...
одного физического диска), FAR2M учитывает ~символические ссылки~@HardSymLink@.
...
$ #Копирование: запрос действия#
    Если одноимённый файл уже существует в папке-приёмнике, FAR2M предложит на выбор
...
ссылок у файла сохраняется.
    ...
    FAR2M умеет создавать жёсткие ссылки, отображать их количество для каждого
...
$ #Переключение между экранами#
    FAR2M позволяет открыть несколько копий встроенной программы просмотра и
...
$ #Команды операционной системы#
    FAR2M самостоятельно обрабатывает следующие команды операционной
системы:
...
~переменные среды~@FAREnv@ FAR2M устанавливает при старте самостоятельно.
...
FAR2M передаёт управление командному процессору
...
@FAREnv
$ #Переменные среды#
    После запуска FAR2M выставляет следующие переменные среды, доступные
дочерним процессам:

    #FARHOME#            каталог, из которого был запущен FAR2M.
    ...
    #FARADMINMODE#       ^<wrap>равна "1", если FAR2M запущен от имени администратора
...
а #d# идёт за выражением нулевого размера.
 ...
 #(?{name}pattern)# - ^<wrap>именованная группа.
...
$ #Регулярные выражения для замены#
    В строке "Заменить на" можно использовать следующие специальные выражения для замены:

    #$N# или #${N}# (где N — одна или более десятичных цифр)
    Они представляют собой номера захваченных групп и заменяются соответствующим найденным текстом.
    Номера присваиваются группам в соответствии с порядком открывающих скобок в выражении.

    #$0# обозначает всю найденную последовательность целиком.

    #${name}# обозначает именованную группу с именем "name".

    Для использования символов #$# и #\# в их буквальном значении,
    их необходимо экранировать обратной косой чертой (т. е. #\$# и #\\\\#).

    #\n# и #\r# интерпретируются как переводы строк, автоматически адаптируясь к стилю
    перевода строк, используемому в файле. Они работают одинаково.

    #\t# заменяется символом табуляции (0x09).

    #\xhhhh# (где hhhh это последовательность от 1 до 4 шестнадцатеричных цифр).
    Это выражение будет заменено символом с указанным значением.
    Например, \x50 будет заменено символом P.

    #\L# - преобразовать следующий текст в нижний регистр
    #\U# - преобразовать следующий текст в верхний регистр
    #\l# - преобразовать следующий 1 символ в нижний регистр
    #\u# - преобразовать следующий 1 символ в верхний регистр
    #\E# - завершить область действия предыдущей директивы \L, \U, \l, \u
...
#~~/.config/far2m/Macros/scripts#. Подробнее см. в документе #$FARHOME/Manuals/macroapi_manual_linux.chm#.
...
$ #Макросы: области действия#
    FAR2M позволяет создавать независимые ~макросы~@KeyMacro@ (макросы, у которых
...
$ #Макросы: кнопки назначения#
    ...
    3. на сочетание клавиши с двумя модификаторами.
       FAR2M позволяет использовать следующие двойные модификаторы:
...
$ #Макросы: запись и воспроизведение#
    FAR2M может выполнять ~макросы~@KeyMacro@ в двух основных режимах:
...
клавиш.
    ...
    1. Начало записи макроса.
       ...
       После того как FAR2M начал запись макроса в левом верхнем
       углу экрана будет отображаться символ '\4FR\-'.

    2. Текст макроса.

       FAR2M будет сохранять все нажимаемые клавиши со следующими
       исключениями:

       - будут сохранены только клавиши, обрабатываемые FAR2M.
         Т.е. если вы в процессе записи макроса из консоли FAR2M
         запустите внешнюю программу, то FAR2M сохранит только
...
хотите удалить.
    ...
    если это сочетание клавиш до записи макроса обрабатывалось FAR2M
...
появившемся диалоге:
    ...
    Если отключить этот параметр, то во время выполнения макрокоманды FAR2M
...
завершения макрокоманды.
    ...
    Позволяет выполнять макрокоманду сразу после запуска FAR2M.
...
макрокоманду:
   ...
   Примечания:
    ...
    1. FAR2M, перед активацией макрокоманды, проверяет все описанные выше
условия.
...
@KeyMacroLang
$ #Макросы: макроязык#
    В FAR2M реализован примитивный макроязык, который позволяет в простую
последовательность клавиш ввести логику, что делает макросы (вместе с
~плагинами~@Plugins@) мощным средством, помогающим в работе с FAR2M.
...
@FarAbout
$ #About FAR#
 Запускается командой #far:about#

 Отображает информацию о Far Manager, его плагинах и системном окружении.

 #Ctrl+C# копирует информацию в буфер обмена.
...
Параметры показаны в виде единого списка, состоящего из четырёх полей:
  ...
  #-# Тип (boolean, 3-state, integer, string)
  #-# Сохраняется ли данная опция при сохранении конфигурации Far (s) или нет (-)
  #-# Значение параметра (для типа integer дополнительно отображается 16-ричное представление)
