...
Для соблюдения привычного пользовательского опыта far2m запускает приложения внутри поддериваемой интерактивной оболочки командной строки.
...
единственным безопасным методом удаленного запуска far2m на не доверенном сервере с поддержкой всех привычных
