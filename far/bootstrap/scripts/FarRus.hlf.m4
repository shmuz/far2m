...
обратной косой чертой (т. е. #\$# и #\\\\#).
    ...
    Как #\n#, так и #\r# интерпретируются как переводы строк, автоматически адаптируясь к стилю
перевода строк, используемому в файле. Они работают одинаково.

    #\t# заменяется символом табуляции (0x09).

    #\xNN# (где N — шестнадцатеричная цифра)
    Это выражение будет заменено байтом с указанным значением.
