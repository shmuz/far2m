...
Для ~раскраски~@Highlight@ всех архивов, кроме "*.rar", необходимо указать последовательность #<arc>|*.rar#.
 ...
 Эти настройки находятся в файле #~~/.config/far2m/settings/masks.ini# в секции #[Masks]#, например:
 [Masks]
 arc=*.rar,*.zip,*.[zj],*.[bg7]z,*.[bg]zip,*.tar

 Пользовательский интерфейс пока не реализован, этот файл создаётся пользователем самостоятельно.
...
Параметры показаны в виде единого списка, состоящего из четырёх полей:
  ...
  #-# Значение параметра (для типа integer дополнительно отображаются 16-ричное и символьное представления).
