...
@About
$ # FAR: що це таке #
    #FAR2M# - це програма управління файлами, що працює в текстовому режимі, яка
має великий набір додаткових функций.

    #FAR2M# є #freeware# і #open source# програмою, що розповсюджується
під GNU GPL v2 ~ліцензією~@License@.

    #FAR2M# дозволяє працювати з #архівами#. При цьому файли в архівах
обробляються аналогічно файлам у папках. #FAR2M# сам перетворює ваші команди на
відповідні дзвінки зовнішніх архіваторів.

    #FAR2M# також забезпечує значну кількість сервісних функцій.

  - FAR2M офіційний сайт
...
@CmdLine
$ # FAR2M: параметри командного рядка#
  Ви можете задавати наступні аргументи в командному рядку або змінному оточенні FAR2M_ARGS:

  #--tty#
  Запускає far2m відразу в ~режимі TTY~@UIBackends@ замість автоматичного визначення можливості
...
досвід, іноді може знадобитися запуск строго в термінальному режимі.
  ...
  #--nodetect#
  За промовчанням far2m намагається на запуску визначити, чи не працює він у терміналі іншого
far2m. У цьому випадку far2m автоматично використовує режим TTY із розширеннями терміналу far2m. В
у разі відсутності таких розширень терміналу - far2m перевіряє наявність доступу до X11 сесії та
...
розширень і дозволяє використовувати лише базові можливості звичайного термінала в TTY режимі.
  ...
  #--mortal#
  Цей аргумент застосовується лише для far2m, який працює в режимі TTY. За замовчуванням про закриття
термінала far2m продовжує працювати 'у фоні' і при наступному запуску іншого far2m у режимі TTY
буде запропоновано активувати far2m, що пішов у фон, замість запуску нової копії програми. Цей ключ
командного рядка півночі відключає цю функціональність, так що закриття терміналу буде наводити
для завершення роботи far2m, що працює в ньому.

  #--primary-selection#
  Використовувати PRIMARY selection замість CLIPBOARD X11 selection. Цей аргумент застосовний тільки для
far2m, який працює у режимі WX.

  #-a#
  Заборона відображення символів з кодами 0 - 31 та 255. Може бути корисним при
запуску FAR2M із telnet.
...
шляхи пошуку можна використовувати змінні середовища.
  ...
  #-co#
  Примушує FAR2M завантажувати зовнішні модулі лише з кешу. Завантаження при цьому відбувається
...
треба завантажити FAR2M без цього параметра. Якщо кешу немає, то модулі не завантажуватимуться.

  Зауваження щодо ключів -p та -co:

  - ^<wrap>якщо вказано порожній -p, то FAR2M запуститься без плагінів;
...
"~Шлях до персональних плагінів~@PluginsManagerSettings@"
  ...
  #-m#
  При старті FAR2M не завантажуватиме макрокоманди з реєстру.

  #-ma#
  При старті FAR2M не виконуватиме макрокоманди з опцією "Виконувати після запуску FAR2M".

  #-u <username>#
  Дозволяє використовувати різні настройки для різних користувачів.
  Наприклад: far -u guest

  FAR2M встановить ~змінне середовище~@FAREnv@ "FARSETTINGS" у значення, що дорівнює <username>.
...
у вказаному прикладі), то FAR2M чекатиме завершення вхідного потоку вічно.
Це особливість поточної версії FAR2M.
...
використовуйте #-cd <path>#) або двох команд із префіксом плагіна. Перший шлях для активної
панелі, другий – для пасивної:
  - ^<wrap>для каталогів та архівів FAR2M покаже їх вміст;
...
команда для пасивної панелі (пасивна панель у своїй тимчасово активируется). Односимвольні префікси ігноруються.
  #Приклад:# far2m ma:~~/Downloads/far2m.7z "lua: msgbox(\\"FAR2M\\",\\"Successfully started\\")"
...
FAR2M також дозволяє використовувати числове сортування.
...
помилковій розкладці клавіатури. У цьому випадку FAR2M перекодує набраний текст
...
здійснюється через нього, інакше – засобами FAR2M.
...
Показати ~історію команд~@History@ #Alt-F8#
 ...
 Зміна розміру вікна FAR2M #Alt-F9#
...
сесії FAR2M, використовується кодова сторінка за промовчанням.
...
командою, що виконується програмою чи пакетним файлом.
 ...
 При запуску вмісту командного рядка FAR2M шукає виконуваний
...
FAR2M також вміє обробляти ~колесо миші~@MsWheel@.
...
створення нових команд FAR2M та підтримки додаткових файлових систем. Наприклад,
...
'Plugins', що знаходиться в одній папці з FAR2M.EXE. При виявленні нового
модуля FAR2M зберігає інформацію про нього і згодом завантажує його лише за
...
список доступної допомоги за модулями, натиснувши:
 ...
 #Shift-F2# - в основній допомозі FAR2M
...
Unicode в іменах файлів та в редакторі).
 ...
 автоматично.
 #F3# - показати технічну інформацію щодо модуля.
...
допомоги, то допомога викликатись не буде.
 ...
 Під час виклику контекстної допомоги FAR2M спробує показати тему з ім'ям
...
Unicode в іменах файлів та в редакторі).
 ...
 Клавіша #F3# дозволяє переглянути технічну інформацію по модулю.

@PluginsReviews
$ #Огляд можливостей модулів, що підключаються#
 Оболонка FAR2M настільки тісно злилася зі своїми плагінами, що
говорити про неї і не говорити про плагіни просто безглуздо. Плагіни неймовірно
розширюють можливості FAR2M, доповнюючи їх та модифікуючи.

@Panels
$ #Панелі#
 Зазвичай FAR2M показує дві панелі (ліве та праве вікна) з різною
...
над папками.
 ...
 FAR2M запам'ятовує інформацію про структуру папок у файлі #tree2.far#,
...
перейменування, копіювання) FAR2M відстежує та оновлює його
Внутрішню структуру. Якщо зміни були зроблені не в FAR2M оновити
...
Для приводів компакт-дисків FAR2M вміє визначати тип приводу. Розрізняються
...
@Terminal
$ #Термінал#
    #FAR2M# містить вбудований емулятор терміналу, що дозволяє виконувати програми командного рядка та керувати ними, спостерігаючи їх виведення.
Для дотримання звичного досвіду користувача far2m запускає додатки всередині піддерюваної інтерактивної оболонки командного рядка.
    #Автодоповнення# FAR2M має два незалежні механізми автоматичного доповнення командного рядка. Перший є механізмом автодоповнення,
...
командної оболонки bash може бути активований натисканням SHIFT+подвійний-TAB (швидко двічі натиснути TAB при натиснутому SHIFT).
    #Поведінка команди exit:# введення команди 'exit' призведе до закриття оболонки командного рядка, але не закриє сам додаток far2m, а наступний
...
Якщо ж ви хочете закрити far2m з командного рядка: надрукуйте псевдокоманду 'exit far' - вона буде розпізнана як far2m команда закриття.
...
викликати пошкодження або збереження даних вбитої програми. Якщо far2m працює в TTY, то ви також можете використовувати поєднання Ctrl+Alt+Z щоб помістити far2m у фон, звільнивши термінал, але продовжуючи виконувати поточну команду.
...
панелями можливо натисканням поєднання Ctrl+O або клацанням по лівому верхньому куті вікна.
    #Розширення терміналу FAR2M# хоча FAR2M сам по собі це додаток текстового режиму, воно може працювати в режимі виведення через підсистему
...
але він також може працювати всередині терміналу іншого far2m, отримуючи від цього додаткові можливості, недоступні під 'звичайними' терміналами,
такі як розпізнавання всіх кнопок клавіатури та подій натискання/відпускання кнопок. Також 'хостовий' far2m може надавати доступ до буфера
обміну хоста та механізм повідомлень робочого столу. Ви можете використовувати цю функціональність, запускаючи TTY far2m на SSH сервері всередині SSH сесії
клієнта, запущеного всередині терміналу far2m, що працює в режимі GUI на хості клієнта. Або, що набагато простіше, використовуючи плагін із можливістю
...
@UIBackends
$ #Режими інтерфейсу#
 Залежно від налаштувань білда та доступних можливостей платформи #FAR2M# може виводити
...
- #Режим TTY|F:# виводить інтерфейс термінал створений іншим додатком far2m instance. Це забезпечує рівень
зручність використання аналогічний режиму GUI (якщо термінал наданий GUI версією far2m).

 Якщо ви хочете запустити far2m віддалено з максимальною зручністю, то рекомендується або запустити його із сесії
підключення NetRocks, що дозволяє використовувати режим TTY|F. Якщо це неможливо - рекомендується запускати far2m
...
єдиним безпечним методом віддаленого запуску far2m на не довіреному сервері з підтримкою всіх звичних
...
Вкажіть повний шлях, де FAR2M шукатиме "персональні" плагіни на додаток до "основних". Можна вказати
кілька шляхів пошуку, розділених символами ':' або ';'. Якщо вказати шлях пошуку, можна використовувати змінні середовища.
...
символом "#/#". Наприклад: #$USERDOMAIN/$USER/Folder3#.
...
розділені символом-розділювачем "#; #" або "#, #".
 #-# ^<wrap>Якщо ця опція включена та ім'я папки містить у собі розділювачі
та/або пробільні символи, то воно має бути взяте в лапки.
 #-# ^<wrap>Наприклад, якщо буде введено #/Foo1;"/foo,2;";/foo3#, то
будуть створені папки з іменами: #/Foo1#, #/foo,2;# та #/foo3#.
 #-# ^<wrap>Якщо ім'я папки укладено в лапки, то кожна лапка всередині імені
має бути подвоєна.
...
пункт #Всі ​​кодові сторінки#, то FAR2M використовуватиме для пошуку всі стандартні
...
вставити нове вкладене меню або пункт меню за допомогою #Ins# або #Ctrl+N# та редагувати
...
місцеве меню в папці FAR2M, то воно буде використане замість того, що зберігається в
...
@FileAssoc
$ #Асоціації файлів#
 FAR2M підтримує асоціацію файлів, які дозволяють задавати
...
FAR2M намагається використовувати асоціації Windows для запуску цього типу
файлів.


@FileAssocModify
$ #Налаштування асоціації файлів#
 FAR2M дозволяє задати шість команд, асоційованих з певним типом
...
у ~Системних параметрах~@SystemSettings@, то FAR2M намагається використовувати
...
метасимволи:
    ...
    #!@@!# or #!$!#    Ім'я файлу, що містить імена позначених файлів
 ...
 1. ^<wrap>FAR2M під час обробки метасимволів підставляється лише те, що вони означають
(ім'я файлу, розширення тощо). Жодних додаткових символів (наприклад,
лапок або прізвищ розробників (FAR2M) не підставляється, і ви повинні це зробити
...
каталогів та визначення сумарного розміру файлів у підкаталогах.
 ...
 #Час бездіяльності#
 Завершує роботу FAR2M, якщо протягом зазначеного інтервалу не було натискання клавіш миші або клавіатури,
FAR2M очікував на введення з командного рядка і відсутні фонові екрани редагування або перегляду.

 #Зберігати історію команд#
 Викликає збереження ~історії команд~@History@ перед завершенням та її відновлення після запуску FAR2M.

 #Зберігати історію папок#
 Викликає збереження ~історії папок~@HistoryFolders@ перед завершенням та її відновлення після запуску FAR2M.
...
після запуску FAR2M. Щоб переглянути файли, що входять до цього списку, натисніть #Alt-F11#.
...
в ~асоціаціях файлів~@FileAssoc@FAR2M, буде запущено програму Windows, призначену для обробки цього типу файлів.

 #Автоматично монтувати CDROM#
 Якщо вибрано компакт-диск у меню ~вибору дисків~@DriveDlg@ FAR2M, закриється відкритий лоток приводу компакт-дисків. Вимкніть цю опцію,
якщо автоматичне монтування приводів працює некоректно (це може статися через помилки в драйверах
деяких приводів)

 #Автозапис конфігурації#
 При виході з FAR конфігурація зберігатиметься автоматично, за винятком стану панелей.
...
#папок розширення# не тільки до файлів, але і до папок. При
 ...
 працює так само, як і у FAR2M 1.65.
...
комбінації кнопкосполучення буде змушувати FAR2M ексклюзивно захоплювати
...
миші у верхній правий кут вікна FAR2M (при вибраній опції).
...
включаючи такі змінні:
 - #%State# - поточний стан FAR2M (раніше було заголовком вікна за замовчуванням);
 - #%Ver# - версія FAR2M;
 - #%Platform# - архітектура FAR2M;
 - #%Backend# - режим інтерфейсу FAR2M;
 - #%Host# - ім'я машини;
 - #%User# - ім'я користувача;
 - #%Admin# - слово "Root", якщо FAR2M запущено від імені адміністратора, інакше порожній рядок.


@DialogSettings
$ #Налаштування діалогів#
 #Історія у рядках# Зберігати історію у рядках введення
#введення діалогів# деяких діалогів FAR2M. Список раніше
...
Ця опція дозволяє змінити ~формат запрошення~@CommandPrompt@ командного рядка.
 ...
 Якщо оболонка не задана, far2m спробує використати системну (#$SHELL#). Якщо системна оболонка
не відповідає внутрішнім критеріям far2m, буде використовуватися #bash#.
...
а робота з іншими командними оболонками може мати значні обмеження або помилки.
 ...
 Показувати текстовий блок із вказівкою far2m-версії, повідомленнями про авторські права
та підказками щодо клавіатури під панелями під час запуску.
...
$ #Формат командного рядка#
 FAR2M дозволяє змінити формат запрошення командного рядка.
...
спеціальні коди:
 ...
 $l - символ <
 $## - символ ## якщо FAR2M запущений під root, інакше $
 ...
 $@ @xx - ^<wrap>"Адміністратор", якщо FAR2M запущено від імені адміністратора.
...
$ #Програма перегляду: клавіші керування#
 ...
 #F4# Переключити текстовий/16-річний режим
    (16-річний режим не підтримує UTF-8 та інші багатобайтові кодування.
    і перемикає перегляд на однобайтову кодову сторінку)
 #F5# Переключити сирий/оброблений режим
 #Alt-F5# Друк файлу (використовується плагін "Print Manager").
...
також список ~макроклавіш~@KeyMacroViewerList@, доступних у програмі перегляду.
 ...
 3. У поточній версії FAR2M існує обмеження на максимальне
 ...
 4. FAR2M ~шукає~@ViewerSearch@ перше входження підрядки (#F7#) з початку видимої
...
з'явиться попередження: "Шлях до редагованого файлу не існує~@WarnEditorPath@".
 ...
 2. ^<wrap>При спробі повторного відкриття на редагування файлу FAR2M видасть
...
~Налаштування редактора~@EditorSettings@.
 ...
 #Shift-Клавіші курсору#       Помітка блоку
 #Тягнути мишу, утримуючи#
   #ліву кнопку#               Помітка блоку
 #Ctrl-Shift-Клавіші курсору#  Помітка блоку за словами
 #Alt-Клавіші курсору#         Помітка вертикального блоку
 #Alt-Shift-Клавіші курсору#   Помітка вертикального блоку
 #Alt + тягнути мишу,#
   #утримуючи ліву кнопку#     Помітка вертикального блоку
 #Ctrl-A#                      Помітити весь текст
 #Ctrl-U#                      Зняти помітку з блоку
 #Shift-Ins, Ctrl-V#           Вставити блок з Буфера Обміну
 #Shift-Del, Ctrl-X#           Перемістити блок у Буфер Обміну
 #Ctrl-Ins, Ctrl-C#            Скопіювати блок у Буфер Обміну
 #Ctrl-<Gray +>#               Додати блок до Буфера Обміну
 #Ctrl-D#                      Видалити блок
 #Ctrl-P#                      ^<wrap>Копіювати блок у поточну позицію курсору (тільки в режимі постійних блоків)
 #Ctrl-M#                      ^<wrap>Перенести блок у поточну позицію курсору (тільки в режимі постійних блоків)
...
$ #Редактор: повторне відкриття файлу#
 FAR2M відслідковує всі спроби повторного відкриття в редакторі
...
неіснуючої папки. Перед збереженням файлу FAR2M створить таку папку при
...
збігаються з тими, що FAR2M запам'ятав при останньому зверненні до цього файлу.
...
запитує привілеї суперкористувача.
 ...
 Налаштування меню переходу зберігаються у конфігурації FAR2M.
...
лише для дисків).
 ...
 Якщо увімкнено показ параметрів компакт-дисків (#Ctrl-8#), то FAR2M буде
...
FAR2M передбачена можливість кольорового виділення файлових об'єктів.
...
$ #Автовизначення кодової сторінки#
 FAR2M намагатиметься вибрати правильну кодову сторінку для перегляду/редагування
...
описи, або дозволено завжди. За промовчанням FAR2M встановлює атрибут "Hidden"
...
описів.
 ...
 Якщо файл з описами має атрибут "Тільки для читання", FAR2M не
...
#описів з атрибутом "Тільки для читання"# FAR2M спробує коректно оновити
описи.

 Якщо це дозволено в конфігурації, FAR2M оновлює опис файлів при
...
FAR2M завжди використовує цей регістр.
...
(кому або крапку з комою), щоб така маска не була поплутана зі списком.
 ...
 Якщо маска укладена у лапки, то кожна лапка всередині маски має бути подвоєна.
...
способів копіювання зустрічених символічних посилань:
 ...
 - Або far2m перевірятиме шлях, який вказується кожним посиланням щоб зрозуміти -
...
системну функцію "копіювання" з діалогу ~Системні параметри~@SystemSettings@.
 ...
 Параметр "Вже існуючі файли" визначає поведінку FAR2M для випадку,
...
одного фізичного диска), FAR2M враховує ~символічні посилання~@HardSymLink@.
...
$ #Копіювання: запит дії#
 Якщо однойменний файл вже існує у папці-приймачі, FAR2M запропонує на вибір
кілька можливих дій:
...
посилань у файлу зберігається.
 ...
 FAR2M вміє створювати жорсткі посилання, відображати їх кількість для кожного
...
$ #Переключення між екранами#
 FAR2M дозволяє відкрити кілька копій вбудованої програми перегляду та
...
$ # Команди операційної системи #
 FAR2M самостійно обробляє наступні команди операційної
системи:
...
~змінні середовища~@FAREnv@ FAR2M встановлює при старті самостійно.
...
FAR2M передає управління командному процесору
операційна система.
...
@FAREnv
$ #Змінні середовища #
 Після запуску FAR2M виставляє наступні змінні середовища, доступні
дочірнім процесам:

 #FARHOME# каталог, з якого було запущено FAR2M.

 #FARLANG# назва поточної мови інтерфейсу

 #FARSETTINGS# ^<wrap>ім'я користувача, вказане у параметрі -u ~командного рядка~@CmdLine@

 #FARADMINMODE# ^<wrap>рівна "1", якщо FAR2M запущений від імені адміністратора
...
$ #Регулярні вирази для заміни#
    У рядку "Замінити на" можна використовувати наступні спеціальні вирази для заміни:

    #$N# або #${N}# (де N — одна або більше десяткових цифр)
    Вони являють собою номери захоплених груп і замінюються відповідним знайденим текстом.
    Номери присвоюються групам відповідно до порядку відкриваючих дужок у виразі.

    #$0# позначає всю знайдену послідовність цілком.

    #${name}# позначає іменовану групу з ім'ям "name".

    Для використання символів #$# та #\# у їхньому буквальному значенні,
    їх необхідно екранувати зворотною косою рискою (тобто #\$# та #\\\\#).

    #\n# та #\r# інтерпретуються як переводи рядків, автоматично адаптуючись до стилю
    переводу рядків, що використовується у файлі. Вони працюють однаково.

    #\t# замінюється символом табуляції (0x09).

    #\xhhhh# (де hhhh це послідовність від 1 до 4 шістнадцяткових цифр).
    Цей вираз буде замінено символом із зазначеним значенням.
    Наприклад, \x50 буде замінено символом P.

    #\L# - перетворити наступний текст у нижній регістр
    #\U# - перетворити наступний текст у верхній регістр
    #\l# - перетворити наступний 1 символ у нижній регістр
    #\u# - перетворити наступний 1 символ у верхній регістр
    #\E# - завершити область дії попередньої директиви \L, \U, \l, \u
...
@KeyMacro
$ #Макроси #
 За замовчуванням макроси завантажуються з файлів з розширеннями #.lua# та #.moon#, що знаходяться в папці
#~~/.config/far2m/Macros/scripts#. Докладніше див. у документі #$FARHOME/Manuals/macroapi_manual_linux.chm#.

 Клавіатурні макрокоманди або макроси - це записана послідовність
натискань клавіш. Макрос дозволяє багаторазово виконувати одну й ту саму
послідовність дій за одне натискання гарячої клавіші.

 Кожен макрос має наступні параметри:
 - ^<wrap>гарячу клавішу, при натисканні на яку буде виконана
записана послідовність;
 - набір додаткових ~параметрів~@KeyMacroSetting@, які впливають на спосіб
і місце відтворення Far записаної послідовності;

 Макроси, найчастіше, використовуються для:
 - ^<wrap>Здійснення однакових дій кілька разів, використовуючи для
виклику послідовності цих дій одне поєднання кнопок.
 - Виконання спеціальних функцій, які задаються спеціальними
командами в тексті макросу.
 - Перевизначення стандартних поєднань клавіш, які
використовує Far для здійснення тих чи інших дій.

 Основним застосуванням макросів є визначення кнопок для швидкого
виклику зовнішніх плагінів і перевизначення дій Far.

 Див. також:

  ~Області дії макросів~@KeyMacroArea@
  ~Кнопки призначення~@KeyMacroAssign@
  ~Запис і відтворення макросів~@KeyMacroRecPlay@
...
@KeyMacroArea
$ #Макроси: області дії#
FAR2M дозволяє створювати незалежні ~макроси~@KeyMacro@ (макроси, у яких
...
~Макрос~@KeyMacro@ може бути призначений на:
 ...
 FAR2M дозволяє використовувати наступні подвійні модифікатори:
...
FAR2M може виконувати ~макроси~@KeyMacro@ у двох основних режимах:
...
клавіш.
 ...
 Після того як FAR2M почав запис макросу у лівому верхньому
 куті екрана відображатиметься символ '\4FR\-'.

 2. Текст макросу.

 FAR2M буде зберігати всі клавіші з наступними
 винятками:

 - будуть збережені лише клавіші FAR2M.
Тобто. якщо ви в процесі запису макросу з консолі FAR2M
 запустіть зовнішню програму, то FAR2M збереже лише
...
виконання, почне виконувати свою початкову функцію. Тобто.
 якщо це поєднання клавіш до запису макросу оброблялося FAR2M
...
з'явився діалог:
 ...
 Якщо вимкнути цей параметр, під час виконання макрокоманди FAR2M
...
завершення макрокоманди.
 ...
 Дозволяє виконувати макрокоманду одразу після запуску FAR2M.
...
макрокоманду:
 ...
 1. FAR2M, перед активацією макрокоманди, перевіряє всі описані вище
умови.
...
@KeyMacroLang
$ #Макроси: макромова#
 У FAR2M реалізована примітивна макромова, яка дозволяє в просту
послідовність клавіш ввести логіку, що робить макроси (разом з
~плагінами~@Plugins@) потужним засобом, що допомагає в роботі з FAR2M.
