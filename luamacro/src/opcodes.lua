return {
  ...
  MCODE_F_CHR=0x380C06; -- S=chr(N)
  MCODE_F_DATE=0x380C07; -- S=date([S])
  MCODE_F_DLG_GETVALUE=0x380C08; -- V=Dlg.GetValue(ID,N)
  MCODE_F_EDITOR_SEL=0x380C09; -- V=Editor.Sel(Action[,Opt])
  MCODE_F_EDITOR_SET=0x380C0A; -- N=Editor.Set(N,Var)
  MCODE_F_EDITOR_UNDO=0x380C0B; -- V=Editor.Undo(N)
  MCODE_F_EDITOR_POS=0x380C0C; -- N=Editor.Pos(Op,What[,Where])
  MCODE_F_EDITOR_DELLINE=0x380C0D; -- N=Editor.DelLine([Line])
  MCODE_F_EDITOR_INSSTR=0x380C0E; -- N=Editor.InsStr([S[,Line]])
  MCODE_F_ENVIRON=0x380C0F; -- S=env(S)
  MCODE_F_FATTR=0x380C10; -- N=fattr(S)
  MCODE_F_FEXIST=0x380C11; -- S=fexist(S)
  MCODE_F_FSPLIT=0x380C12; -- S=fsplit(S,N)
  MCODE_F_FMATCH=0x380C13; -- N=FMatch(S,Mask)
  MCODE_F_IIF=0x380C14; -- V=iif(C,V1,V2)
  MCODE_F_INDEX=0x380C15; -- S=index(S1,S2[,Mode])
  MCODE_F_INT=0x380C16; -- N=int(V)
  MCODE_F_ITOA=0x380C17; -- S=itoa(N[,radix])
  MCODE_F_KEY=0x380C18; -- S=key(V)
  MCODE_F_LCASE=0x380C19; -- S=lcase(S1)
  MCODE_F_LEN=0x380C1A; -- N=len(S)
  MCODE_F_MAX=0x380C1B; -- N=max(N1,N2)
  MCODE_F_MENU_CHECKHOTKEY=0x380C1C; -- N=checkhotkey(S[,N])
  MCODE_F_MENU_GETHOTKEY=0x380C1D; -- S=gethotkey([N])
  MCODE_F_MENU_SELECT=0x380C1E; -- N=Menu.Select(S[,N[,Dir]])
  MCODE_F_MIN=0x380C1F; -- N=min(N1,N2)
  MCODE_F_MOD=0x380C20; -- N=mod(a,b) == a %  b
  MCODE_F_MLOAD=0x380C21; -- B=mload(var)
  MCODE_F_MSAVE=0x380C22; -- B=msave(var)
  MCODE_F_MSGBOX=0x380C23; -- N=msgbox(["Title"[,"Text"[,flags]]])
  MCODE_F_PANEL_FATTR=0x380C24; -- N=Panel.FAttr(panelType,fileMask)
  MCODE_F_PANEL_SETPATH=0x380C25; -- N=panel.SetPath(panelType,pathName[,fileName])
  ...
  MCODE_F_FAR_CFG_GET=0x380C5C; -- V=Far.Cfg_Get(Key,Name)
